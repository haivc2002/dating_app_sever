...
const getInfo = async (req, res) => {
  try {
    ...
    if (!idUser) {
      return res.status(400).json({ result: 'Error', message: 'idUser is required' });
    }

    const userInfo = await dbGet('SELECT * FROM info WHERE idUser = ?', [idUser]);
    const images = await dbAll('SELECT * FROM listImage WHERE idUser = ?', [idUser]);
    const infoMore = await dbGet('SELECT * FROM infoMore WHERE idUser = ?', [idUser]);
